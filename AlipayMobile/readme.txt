...
WS_WAP_PAYWAP-CSHARP-UTF-8
  ...
  │
  ├call_back_url.aspx ┈┈┈┈┈页面跳转同步通知文件
  ├call_back_url.aspx.cs┈┈┈┈页面跳转同步通知文件
  │
...
功能：根据字母a到z的顺序把参数排序
输入：Dictionary<string, string> dicArrayPre 排序前的参数组
输出：Dictionary<string, string> 排序后的参数组
...
功能：验证消息是否是支付宝发出的合法消息，验证callback
...
功能：验证消息是否是支付宝发出的合法消息，验证服务器异步通知
输入：SortedDictionary<string, string> inputPara 通知返回参数数组
      string sign 支付宝生成的签名结果
输出：bool 验证结果
...
功能：解密
输入：Dictionary<string, string> inputPara 要解密数据
输出：Dictionary<string, string> 解密后结果
...
输入：string GATEWAY_NEW 支付宝网关地址
      Dictionary<string, string> sParaTemp 请求参数数组
...
功能：建立请求，以模拟远程HTTP的POST请求方式构造并获取支付宝的处理结果
输入：string GATEWAY_NEW 支付宝网关地址
      SortedDictionary<string, string> sParaTemp 请求参数数组
      string strMethod 提交方式。两个值可选：post、get
...
功能：解析远程模拟提交后返回的信息
输入：string strText 要解析的字符串
输出：Dictionary<string, string> 解析结果
